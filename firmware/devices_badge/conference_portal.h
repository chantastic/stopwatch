...
class ConferencePortal {
  ...
  void syncClock(JsonDocument &input){
    // A timezone label is informational only. The checked numeric offset is
    // authoritative; neither the browser nor the badge applies DST rules here.
    for(JsonPair field:input.as<JsonObject>()){
      const char *key=field.key().c_str();
      if(strcmp(key,"epoch")&&strcmp(key,"offset_minutes")&&strcmp(key,"timezone")){respond(400,"Unexpected clock field.");return;}
    }
    if(!input["timezone"].isNull()){
      if(!input["timezone"].is<const char*>()){respond(400,"Invalid timezone label.");return;}
      JsonString zone=input["timezone"].as<JsonString>();
      if(!zone.size()||zone.size()>64||zone.size()!=strlen(zone.c_str())){respond(400,"Invalid timezone label.");return;}
      for(size_t i=0;i<zone.size();i++){
        char c=zone.c_str()[i];
        if(!conferenceAlnum(c)&&c!='/'&&c!='_'&&c!='-'&&c!='+'){respond(400,"Invalid timezone label.");return;}
      }
    }
    JsonDocument result;bool synchronized=conferenceClockSyncPhone(input,result);
    result["message"]=synchronized?"Clock synchronized from this browser. Profile edits are saved separately.":"Clock sync could not be verified. Check your device time and try again.";
    String body;serializeJson(result,body);
    transport_.reply(synchronized?200:400,"application/json",body);
  }
  void submit() {
    ...
    if(uri=="/cancel"){
      respond(200,"Profile changes cancelled. Any successful clock sync is kept. You may close this page.");outcome_=ConferencePortalOutcome::Cancelled;closeScheduled_=false;return;
    }
    ...
    if(uri=="/clock"){syncClock(json);return;}
